...
export default function AdminTouristAreaPage() {
    ...
    const fetchAreas = async () => {
        setLoading(true);
        try {
            const res: any = await adminApi.getAllTouristAreas(
                currentPage,
                10,
                searchTerm,
                statusFilter,
    ...
    const handleDelete = async (id: number) => {
        if (
            !confirm(
                `Sếp có chắc chắn muốn XÓA VĨNH VIỄN khu du lịch #${id} không?`,
            )
        )
            return;
        ...
        setActionLoading(id);
        try {
            const res = await adminApi.deleteTouristArea(id);
            if (res.success) {
    ...
    const handleApprove = async (id: number, status: string) => {
        ...
        if (!confirm(`Sếp muốn ${actionText} khu du lịch này?`)) return;

        setActionLoading(id);
        try {
            const res = await adminApi.approveTouristArea(id, status);
            if (res.success) {
                alert(`✅ ${res.message}`);
                fetchAreas();
            } else {
    ...
    const handleCloseModal = () => {
        ...
        setEditingData(null);
        fetchAreas(); // Load lại data sau khi sửa
    };
                ...
                <div className="flex items-center">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">
                            Tổng cộng: {totalCount} khu du lịch
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    <div className="relative w-full sm:w-64">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none"
                            value={searchTerm}
                            onChange={(e) => {
                    ...
                    <select
                        className="px-4 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 bg-white"
                        value={statusFilter}
                        onChange={(e) => {
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr>
                                    ...
                                    >
                                        Không tìm thấy khu du lịch nào!
                                    </td>
                                </tr>
                            ) : (
                                areas.map((area) => (
