"use client";

import { useLoading } from "@/context/LoadingContext";
import Image from "next/image";
import dynamic from "next/dynamic";
import { useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";
import { touristPlaceDetailApi } from "@/app/api/touristPlaceDetailApi";
import { ListTour } from "@/components/layouts/ListTour";
import { ListHottel } from "@/components/layouts/ListHottel";
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation, Pagination, Keyboard } from "swiper/modules";

// Load MapView động
const MapView = dynamic(() => import("@/components/layouts/MapView"), {
    ssr: false,
    loading: () => <p>Đang tải bản đồ...</p>,
});

const TouristPlaceDetail = () => {
    const searchParams = useSearchParams();
    const placeId = Number(searchParams.get("id"));
    const { setLoading } = useLoading();
    const [isOpen, setIsOpen] = useState(false);

    // State lưu thông tin địa điểm (để lấy tọa độ set map)
    const [placeDetail, setPlaceDetail] = useState<any>(null);

    // State lưu danh sách (Khách sạn hoặc Tour tùy tab)
    const [listData, setListData] = useState<any[]>([]);

    // State điều khiển giao diện
    const [typeDetail, setTypeDetail] = useState<"Hotel" | "Tour">("Hotel");
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [totalPages, setTotalPages] = useState<number>(0);
    const [selectedLocation, setSelectedLocation] = useState<
        [number, number] | null
    >(null);

    const fetchData = async () => {
        if (!placeId) return;
        try {
            setLoading(true);
            const res = await touristPlaceDetailApi({
                id: placeId,
                type: typeDetail,
                page: currentPage,
                pageSize: 10,
            });

            if (res.success) {
                // Set thông tin chung của địa điểm (chỉ cần set 1 lần)
                if (!placeDetail) setPlaceDetail(res.tourist_Place_Detail);

                // Set data cho List
                setListData(res.pagedResult?.items ?? []);
                console.log(listData);
                setTotalPages(res.pagedResult?.totalPages ?? 0);
            }
        } catch (err) {
            // Xử lý lỗi (nếu BE ném Exception)
            setListData([]);
            setTotalPages(0);
        } finally {
            setLoading(false);
        }
    };

    // Reset lại page = 1 khi đổi tab
    useEffect(() => {
        setCurrentPage(1);
        setSelectedLocation(null);
    }, [typeDetail]);

    // Gọi API mỗi khi page hoặc type thay đổi
    useEffect(() => {
        fetchData();
    }, [currentPage, typeDetail, placeId]);

    return (
        <div className="min-h-screen bg-gradient-to-br from-zinc-50 via-white to-zinc-100 flex justify-center">
            {/* ĐÃ FIX: Thêm lại lg:flex-row và tăng max-w-7xl để bung rộng toàn màn hình */}
            <main className="flex flex-col lg:flex-row w-full max-w-7xl gap-6 py-6 px-3 sm:px-4 md:px-6">
                {/* CỘT TRÁI: ĐÃ FIX - Tăng độ rộng từ 420px lên hẳn 550px (đến 600px) cho thẻ Tour bung xõa */}
                <div className="w-full lg:w-[550px] xl:w-[600px] bg-white rounded-3xl shadow-lg p-5 flex flex-col max-h-[85vh] overflow-hidden border border-zinc-100">
                    {/* Header thông tin địa điểm */}
                    <div className="mb-4 shrink-0">
                        <h1 className="text-2xl font-bold text-zinc-800">
                            📍 {placeDetail?.name || "Đang tải..."}
                        </h1>
                        <p className="text-sm text-zinc-500 line-clamp-2 mt-1">
                            {placeDetail?.description}
                        </p>
                    </div>

                    {/* TABS CHUYỂN ĐỔI HOTEL / TOUR */}
                    <div className="flex gap-2 mb-4 bg-zinc-100 p-1 rounded-xl shrink-0">
                        <button
                            onClick={() => setTypeDetail("Hotel")}
                            className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${
                                typeDetail === "Hotel"
                                    ? "bg-white shadow text-blue-600"
                                    : "text-zinc-500 hover:text-zinc-700"
                            }`}
                        >
                            🏨 Khách sạn
                        </button>
                        <button
                            onClick={() => setTypeDetail("Tour")}
                            className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${
                                typeDetail === "Tour"
                                    ? "bg-white shadow text-blue-600"
                                    : "text-zinc-500 hover:text-zinc-700"
                            }`}
                        >
                            🚌 Tour du lịch
                        </button>
                    </div>

                    {/* HIỂN THỊ DANH SÁCH SẢN PHẨM */}
                    <div className="flex-1 overflow-y-auto pr-2 space-y-4 scrollbar-thin scrollbar-thumb-zinc-300">
                        {listData.length > 0 ? (
                            typeDetail === "Tour" ? (
                                <ListTour
                                    data={listData}
                                    totalPages={totalPages}
                                    setSelectedLocation={setSelectedLocation}
                                    currentPage={currentPage}
                                    setCurrentPage={setCurrentPage}
                                />
                            ) : (
                                <ListHottel
                                    data={listData}
                                    totalPages={totalPages}
                                    setSelectedLocation={setSelectedLocation}
                                    currentPage={currentPage}
                                    setCurrentPage={setCurrentPage}
                                />
                            )
                        ) : (
                            <div className="text-center py-10 text-zinc-500 text-sm">
                                Hiện chưa có{" "}
                                {typeDetail === "Hotel" ? "khách sạn" : "tour"}{" "}
                                nào.
                            </div>
                        )}
                    </div>
                </div>

                {/* CỘT PHẢI: THÔNG TIN ĐỊA ĐIỂM + MAP VIEW */}
                <div className="flex-1 flex flex-col gap-4 h-auto lg:h-[calc(100vh-100px)] lg:sticky lg:top-20">
                    {/* CỤC THÔNG TIN TOURIST PLACE (ĐÃ FIX: CLICK MỞ GALLERY ẢNH) */}
                    {placeDetail && (
                        <div className="w-full bg-white rounded-3xl shadow-md border border-zinc-100 p-5 shrink-0 flex gap-5 items-start">
                            {/* Ảnh đại diện - ĐÃ FIX: TĂNG KÍCH THƯỚC VÀ CLICK ĐỂ MỞ GALLERY */}
                            <div
                                // Chỗ này click để mở Modal nha bro
                                onClick={() => setIsOpen(true)}
                                // ĐÃ FIX: Tăng kích thước từ w-28 lên w-36 (khoảng 144px) cho ảnh to ra
                                className="relative w-36 h-36 md:w-40 md:h-40 shrink-0 rounded-2xl overflow-hidden shadow-sm border border-zinc-100 cursor-pointer group"
                            >
                                <Image
                                    src={
                                        placeDetail.coverImageUrl ||
                                        "/Img/ImgNull.jpg"
                                    }
                                    alt={placeDetail.name}
                                    fill
                                    className="object-cover group-hover:scale-110 transition-transform duration-700"
                                />
                                {/* Overlay mờ mờ khi hover vào */}
                                <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white text-2xl">
                                    🖼️
                                </div>
                            </div>

                            {/* Cột chữ chi tiết (cũng to ra chút cho tương xứng với ảnh) */}
                            <div className="flex flex-col flex-1 gap-1.5 pt-1">
                                <h2 className="text-2xl font-black text-zinc-900 leading-tight">
                                    {placeDetail.name}
                                </h2>
                                <p className="text-base text-zinc-600 line-clamp-2 leading-relaxed flex items-start gap-1.5 mt-1">
                                    <span className="mt-0.5">📍</span>
                                    <span>
                                        {placeDetail.address ||
                                            "Đang cập nhật địa chỉ..."}
                                    </span>
                                </p>

                                {/* Thống kê Rating / Favorite / View (Cái này giữ nguyên nè) */}
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3">
                                    <div className="flex items-center gap-1.5 text-sm font-bold text-zinc-800 bg-zinc-50 px-2.5 py-1 rounded-lg border border-zinc-100">
                                        <span className="text-yellow-500">
                                            ⭐
                                        </span>
                                        {placeDetail.rating_average
                                            ? placeDetail.rating_average.toFixed(
                                                  1,
                                              )
                                            : "5.0"}
                                    </div>
                                    <div className="flex items-center gap-1.5 text-sm font-bold text-zinc-800 bg-zinc-50 px-2.5 py-1 rounded-lg border border-zinc-100">
                                        <span className="text-red-500">❤️</span>
                                        {placeDetail.favorite_count || 0} lượt
                                        thích
                                    </div>
                                    <div className="flex items-center gap-1.5 text-sm font-bold text-zinc-800 bg-zinc-50 px-2.5 py-1 rounded-lg border border-zinc-100">
                                        <span className="text-blue-500">
                                            🔥
                                        </span>
                                        {placeDetail.click_count || 0} người xem
                                    </div>
                                </div>
                            </div>
                        </div>
                    )}

                    {/* BẢN ĐỒ BÊN DƯỚI (Dùng flex-1 để nó tự kéo dãn) */}
                    <div className="w-full flex-1 rounded-3xl overflow-hidden shadow-md border border-zinc-200 bg-white relative min-h-[300px]">
                        {placeDetail ? (
                            <MapView
                                locations={listData}
                                selectedLocation={selectedLocation}
                                LocaltionSetView={[
                                    placeDetail.latitude ?? 15.8,
                                    placeDetail.longitude ?? 105.8,
                                ]}
                                size={12}
                            />
                        ) : (
                            <div className="w-full h-full bg-zinc-50 animate-pulse flex items-center justify-center text-zinc-400">
                                Đang tải bản đồ...
                            </div>
                        )}
                    </div>
                </div>

                {/* MODAL XEM ẢNH FULL MÀN HÌNH (LIGHTBOX) - ÔP NGUYÊN TỪ HOTEL DETAIL SANG 😎 */}
                {isOpen && (
                    <div
                        className="fixed inset-0 z-[999] bg-black/95 flex items-center justify-center backdrop-blur-sm transition-opacity p-4"
                        onClick={() => setIsOpen(false)} // Click vào vùng tối là đóng liền
                    >
                        {/* Nút Đóng (X) */}
                        <button
                            onClick={() => setIsOpen(false)}
                            className="absolute top-4 right-4 text-white text-4xl hover:text-gray-400 transition z-[1000] p-2"
                        >
                            &times;
                        </button>

                        {/* Wrapper chứa Swiper */}
                        <div
                            className="w-full max-w-6xl h-full flex items-center justify-center"
                            onClick={(e) => e.stopPropagation()} // Click vào ảnh không bị đóng modal
                        >
                            <Swiper
                                modules={[Navigation, Pagination, Keyboard]}
                                navigation
                                pagination={{
                                    clickable: true,
                                    dynamicBullets: true,
                                }}
                                keyboard={{ enabled: true }}
                                grabCursor={true}
                                className="w-full h-[80vh] rounded-lg"
                            >
                                {/* Ở đây tui phải tự mường tượng cái mảng ảnh, nếu chưa có thì chỉ hiện 1 tấm cover nha */}
                                {(placeDetail.images?.length
                                    ? placeDetail.images
                                    : [
                                          placeDetail.coverImageUrl ||
                                              "/Img/ImgNull.jpg",
                                      ]
                                ).map((src: string, index: number) => (
                                    <SwiperSlide key={index}>
                                        <div className="relative w-full h-full flex items-center justify-center">
                                            <Image
                                                src={src}
                                                alt={`Place image ${index + 1}`}
                                                fill
                                                className="object-contain" // Contain để không mất góc nha bro
                                            />
                                        </div>
                                    </SwiperSlide>
                                ))}
                            </Swiper>
                        </div>
                    </div>
                )}
            </main>
        </div>
    );
};

export default TouristPlaceDetail;
